...
function createWall(models) {
  for (let i = 0; i < models.length; i++) {
    ...
    wall = new THREE.Mesh(
      ...
      new THREE.MeshPhongMaterial({ color: 0x42f5bf, visible: false })
    );
    ...
    const gridHelper = new THREE.GridHelper(
      Math.max(wall.scale.x, wall.scale.y, wall.scale.z),
      20,
      0x1111aa,
      0xaa1111
    );
...
function setupCamera(cam) {
  ...
  let ball = (ballObject = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({ color: 0x05ff1e })
  ));
